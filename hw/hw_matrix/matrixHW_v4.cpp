...
void equationSolver(float **A, float x, float **B, int n)
{
	std::cout << "Equation Solver: (A*3)-A" << std::endl;
	for (int i = 0; i < n; i++)
	  {
	    for (int j = 0; j < n; j++)
	    {
	      std::cout << (A[i][j] * x) - B[i][j] << "  ";
	    }
	    std::cout << std::endl;
	  }
	  std::cout << std::endl;
...
*/
  ...
  // A^2 and A^3						------ Matrix Multiplication --------
  ...
  // Equation Solver: A*x-B				------ Equation Solver --------
  equationSolver(A, 3, A, n); // (A*3)-A

  // Print Grahm-Schmidt				------ Grahm-Schmidt --------
